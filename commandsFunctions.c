...
void caseSingle(void){
	...
	/* 	need a ptr to the adcsamples returned pointer,
			so each function uses the same adc samples array */
  byte *ptr;
  ...
  if (SCI1S1_RDRF){
    ...
    switch (temp){
      ...
      case 0x07:
        ...
        //setColRgb(0, 0xff, 0); /* set to green */
        Cls(); /* clear screen a fill with color abvoe */
        break;
      case 0x08:
        lightOn();
        break;
